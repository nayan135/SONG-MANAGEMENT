...
if ($result->num_rows > 0) {
   ...
   json_encode($audioPaths);  // Return the data as JSON
} else {
    echo "0 results";
}
...
<body>
 <div class="song-tile" onclick="showCard()" id="1">
    <div class="tile__img" id="tileImage1"></div>
    <div class="tile__info">
        <div class="tile__title" id="tileSongName1">Song Name</div>
        <div class="tile__subtitle" id="tileArtistName1">Artist</div>

    </div>
</div>
 
<!-- Second Song Tile -->
<div class="song-tile" onclick="showCard()" id="2">
    <div class="tile__img" id="tileImage2"></div>
    <div class="tile__info">
        <div class="tile__title" id="tileSongName2">Song Name</div>
        <div class="tile__subtitle" id="tileArtistName2">Artist</div>
    </div>
</div>
   ...
   <audio id="audioPlayer" src="<?php echo $audioPaths[0]; ?>"></audio>
   ...
   <script>
        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                ...
                songs.forEach(song => {
                    ...
                    tileImage.style.backgroundImage = `url('${song.audio_path}')`;
    ...
    function loadDetailsAndPlay(song) {
            // Extract details from the song path
            const fileName = song.audio_path.split('/').pop();
            const nameWithoutExtension = fileName.split('.')[0];
            const [songName, artistName] = nameWithoutExtension.split('--').map(str => str.trim());

            // Set details in the card
            document.getElementById('songName').textContent = songName;
            document.getElementById('artistName').textContent = artistName;

            // Set background image for .card__img
